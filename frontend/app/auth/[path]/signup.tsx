...
export default function SignUp() {
    ...
    const showErr = (field: 'email' | 'password' | 'passwordRepeat' | 'policyAgree') => {
        if (!result?.success && result?.error?.[field]) {
            const errmsg = result.error[field]
            return (
                <p className="text-red-400 mb-2 text-sm pl-2 flex">
                    <VscErrorSmall size={20} />
                    <span>{errmsg}{result?.error?.emailAlreadyExist && (<> Try <a href="/auth/login" className='text-white'>Login</a> or <a href="#" className='text-white'>Reset Password</a></>)} </span>
                </p>
            )
        }
    }
    return (
        <>
            <section className="relative h-screen py-20 2xl:py-40 bg-red-300 overflow-hidden">
                {result?.error?.internalError && (<Alert />)}
                <div className="relative container px-4 mx-auto">
                    <div className="max-w-5xl mx-auto">
                        <div className="flex flex-wrap items-center -mx-4">
                            <div className="w-full lg:w-1/2 px-4 mb-16 lg:mb-0">
                                <div className="max-w-md">
                                    <h2 className="mt-8 mb-12 text-5xl font-bold font-heading text-black">Start your journey by creating an account.</h2>
                                </div>
                            </div>
                                ...
                                <div className="px-6 lg:px-20 py-12 lg:py-24 bg-gray-700 rounded-lg">
                                    <form action={formAction}>
                                        ...
                                        <div className="flex items-center pl-6 mb-3 bg-white rounded-full">
                                            ...
                                            <input className="w-full pl-4 pr-6 py-4 font-semibold placeholder-gray-900 placeholder:italic placeholder:font-light rounded-r-full focus:outline-none" name="email" type="text" placeholder="example@example.com" />
                                        </div>
                                        {showErr('email')}
                                        <div className="flex items-center pl-6 mb-3 bg-white rounded-full">
                                            <span className="inline-block pr-3 py-2 border-r border-gray-50">
                                            ...
                                            <input className="w-full pl-4 pr-6 py-4 font-semibold placeholder-gray-900 placeholder:italic placeholder:font-light rounded-r-full focus:outline-none" name="password" type="password" placeholder="Password" />
                                        </div>
                                        {showErr('password')}
                                        <div className="flex items-center pl-6 mb-3 bg-white rounded-full">
                                            <span className="inline-block pr-3 py-2 border-r border-gray-50">
                                            ...
                                            <input className="w-full pl-4 pr-6 py-4 font-semibold placeholder-gray-900 placeholder:italic placeholder:font-light rounded-r-full focus:outline-none" name="passwordRepeat" type="password" placeholder="Repeat password" />
                                        </div>
                                        {showErr('passwordRepeat')}
                                        <div className="inline-flex mb-3 mt-3">
                                            <input className="mr-4" type="checkbox" name="policyAgree" value="agree" />
